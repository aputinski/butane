...
export function replaceFunctions (value, options) {
  ...
  function walkValue (node, key, parentNode) {
    ...
    if (isCallExpression && includes(fnNames, node.callee.name)) {
      ...
      const body = fn(...args)
      const expression = replaceFunctions(body, options).code
      parentNode[key] = parseExpression(expression).expression
    } else if (isCallExpression && node.callee.name !== undefined) {
      throw new Error(node.callee.name + ' has not been defined')
    } else {
      // Keep walking
...
export function coerceVal (value) {
  ...
  function walk (node) {
    if (node.type === 'MemberExpression' || node.type === 'Identifier') {
      ...
      // Append .val() to computed properties
      collectComputedProperties(node).forEach(walk)
      // Append .val() to CallExpression arguments
      collectArguments(node).forEach(walk)
      // Append .val()
...
export function replaceChildSyntax (value) {
  ...
  function walkValue (node, key, parentNode, parentKey) {
    ...
    if (key === 'property' && (!isCallIdentifier || isParentComputed)) {
      ...
      let argument
      // MemberExprssions
      if (proposedArgument.type === 'MemberExpression' || proposedArgument.type === 'CallExpression') {
