 ...
 * @returns {object}
 */
function getMemberExpressionRootName(node) {
  let name = null;
  function walk(node) {
 ...
 *
 */
function standardWalk(fn, parentKey) {
  return function(child) {
    if (_.isObject(child)) {
      fn.apply(null, _(arguments).slice().push(parentKey).value());
    }
  }
}
...
/**
 * Replace items in the REPLACE_IDENTIFIERS map
 *
 * @param {string} value
 * @returns {object}
 */
function replaceFirebaseIdentifiers(value) {
   ...
   *
   */
  function walk(node, key, parentNode) {
    if (node.type === 'Identifier' && key !== 'property') {
      const name = getMemberExpressionRootName(node);
      if (has(REPLACE_IDENTIFIERS, name)) {
        node.name = REPLACE_IDENTIFIERS[name];
      }
    }
    else {
      forEach(node, standardWalk(walk, key));
    }
  }
  walk(syntax.body[0]);
  return {
